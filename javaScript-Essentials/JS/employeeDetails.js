...
function Specialization(specialization) {
  const specializationEmployees = employees.find(
    (employee) => employee.specialization === specialization,
  );
  if (specializationEmployees) {
    document.getElementById("employeesDetails").innerHTML =
      `<p>${specializationEmployees.id}: ${specializationEmployees.name} - ${specializationEmployees.department} - ${specializationEmployees.salary}</p>`;
  } else {
    document.getElementById("employeesDetails").innerHTML =
      "No employee found with the specified specialization.";
  }
}
